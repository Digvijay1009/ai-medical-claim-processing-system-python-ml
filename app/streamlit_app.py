...
#         # 🧩 Diagnostic print to confirm the correct version of the class is loaded
#         print("🧩 Report Generator Source:", self.report_generator.__class__)
#         print("🧩 PDF methods:", [m for m in dir(self.report_generator) if "pdf" in m.lower()])

#     # ---
#     # --- ⚠️ NEW: Caching Function (This was missing in your script) ---
#     # ---
#     @st.cache_data(ttl=3600, show_spinner=False) 
#     def _get_cached_report(_self, claim_id):
#         """
...
#         # --- FIX: Read from the 'sections' key ---
#         summary_section = report.get('sections', {}).get('executive_summary', {})
#         if not summary_section:
#             st.error("Report structure is invalid. Missing 'executive_summary' section.")
#             return
...
#             treatment_guidelines = medical_section.get('treatment_guidelines_compliance', {})
#             if treatment_guidelines:
#                 unnecessary = treatment_guidelines.get('unnecessary_treatments_found', []) # Check key
#                 if unnecessary:
#                     st.warning("**Unnecessary Treatments:**")
#                     for treatment in unnecessary:
#                         st.write(f"• {treatment}")
...
#         Display detailed fraud claim information
#         """
#         comp_report_sections = comp_report_sections.get('sections', {}) # Ensure we have the sections dict
...
#             if st.button(f"📋 Comprehensive Analysis", key=f"detail_{claim['claim_id']}"):
#                 st.session_state.selected_claim = claim['claim_id']
#                 # We are already on the dashboard, so just re-run to show the selection
#                 st.rerun() 
#         with col2:
#             if st.button(f"🚨 Mark as Fraud", key=f"fraud_{claim['claim_id']}"):
#                 self.db_handler.update_claim_status(claim['claim_id'], "Fraud Suspected", "AI System", "Domain-specific fraud detection")
...
#                             try:
#                                 # --- 
#                                 # This is the *original* report from the pipeline
#                                 # It has a different structure from the report_generator report
#                                 # ---
#                                 comprehensive_report_pipeline = self.pipeline.process_claim_comprehensive(uploaded_files)
#                                 st.session_state.comprehensive_report = comprehensive_report_pipeline
                                
#                                 # --- FIX ---
#                                 # We MUST convert this pipeline report into the standard
#                                 # 'sections' report that _display_comprehensive_results expects.
#                                 # The easiest way is to re-fetch it using the generator.
#                                 claim_id = comprehensive_report_pipeline.get('claim_info', {}).get('claim_id')
#                                 if claim_id:
#                                     # --- ⚠️ USE CACHED REPORT ---
#                                     display_report = self._get_cached_report(claim_id)
...
#                 for claim in claims:
#                     # --- 
#                     # --- THIS IS THE FIX for the main table ---
#                     # ---
#                     df_data.append({
#                         'Claim ID': claim['claim_id'],
#                         'Patient Name': claim['patient_name'] or 'Unknown',
...
#                 # Local function, does not use 'self'
#                 def color_business_decision(val):
#                     if val == "APPROVED":
#                         return 'background-color: #d4edda; color: #155724;'
#                     elif val == "DENIED":
...
#             for claim in claims:
#                 # ---
#                 # --- THIS IS THE FIX for the fraud page ---
#                 # ---
#                 comp_report = {} # Start with an empty report
...
#                 # Read these violations from the flat claim fields
#                 has_coverage_violations = (
#                     claim.get('policy_status') == 'EXPIRED' or
#                     bool(claim.get('policy_limits_exceeded')) or # Check if the list is not empty
#                     bool(claim.get('policy_exclusions'))        # Check if the list is not empty
#                 )
...
#                     except Exception as e:
#                         st.warning(f"Could not generate report for {claim['claim_id']}: {e}")
#                         comp_report = {} # Use an empty report if generation fails
...
#                         expanded=False
#                     ):
#                         # This display function now receives the correctly generated comp_report
#                         self._display_fraud_claim_details(claim, comp_report)
...
#             # --- This page was already fixed, no changes needed ---
#             review_statuses = ['Under Review', 'Pending', 'Fraud Suspected', 'More Info Needed']
#             review_claims = [claim for claim in claims if claim.get('status') in review_statuses]
...
#                         with col3:
#                             if st.button("📋 View Details", key=f"details_review_{claim['claim_id']}"):
#                                 # --- FIX: This button should navigate to the dashboard and select the claim ---
#                                 # For now, it just re-runs, which is fine
#                                 st.session_state.selected_claim = claim['claim_id']
#                                 st.rerun()
...
#                 "Select a Claim",
#                 options=claim_options,
#                 format_func=lambda x: x[0] # Show the display_text
#             )

#             if selected_claim_tuple:
#                 selected_claim_id = selected_claim_tuple[1] # Get the actual claim_id
#                 st.info(f"Ready to generate PDF for **{selected_claim_id}**.")
